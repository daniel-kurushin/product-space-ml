...
clusters = load('data/macroregions.new.json')
...
def is_special(industry, cluster_regions):
...
for cluster in clusters.keys():
    try:
        cluster_regions = set(clusters[cluster]["состав кластера"])
        
        graph = "digraph g {\n\trankdir=LR\n"
        for node in sorted(nodes):
            shape = 'circle' if is_special(node, cluster_regions) else 'plain'
        ...
        mr_name = cluster.replace(' ','_')
        open('data/graph.%s.dot' % (mr_name), 'w').write(graph)
        print('neato -Tsvg "data/graph.%s.dot" > "/tmp/graph.%s.dot.svg"' % (mr_name, mr_name))
    except TypeError:
        pass
