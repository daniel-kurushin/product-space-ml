...
def get_region_properties():
    cluster_data = open_workbook('data/cluster_in_data_9.xls').sheet_by_index(0)
    N = {}
    
    for row in range(1, cluster_data.nrows):
        regname = get_reg_name(cluster_data.cell(row, 0).value.lower().strip())
        
        k = float(cluster_data.cell(row, 1).value)
          
        N.update({
            regname:
                {
                    'Текущие центры экономического роста' : (k, "Экономика"),
                }
            }
        )
    dump(N, 'data/region_properties_9.json')
    return N
...
def sort_regions_by_properties(N):
    ...
    for region in N:
        regname = region
        value = float(np.sum(np.array([ N[region][c][0] for c in N[region] ])))
        out += [(regname, value)]
...
def filter_bad_clusters(region_clusters):
    ...
    for cluster in region_clusters.keys():
        if len(region_clusters[cluster]["состав кластера"]) < 3:
...
dump(region_clusters, 'data/clusters_out_9.json')
